...
- **Enhanced security**: API keys are stored securely in Cloudflare's infrastructure and never exposed in your application code or logs
- **Simplified development**: No need to manage API keys in your application environment or pass them with each request
- **Centralized management**: Update API keys for all your applications from a single location in the dashboard
- **Reduced risk**: Eliminates the possibility of accidentally exposing API keys in client-side code or version control
...
BYOK currently supports the following AI providers:

{/* TODO UPDATE */}

- **OpenAI**: GPT models, DALL-E, Whisper, and other OpenAI services
...
{/* TODO UPDATE IMAGE */}
![Bring your own key](~/assets/images/ai-gateway/bring-your-own-key.png)

:::note[Security best practices]

- API keys are encrypted at rest and in transit
- Keys are never logged or exposed in response headers
- You can rotate keys at any time without code changes
- Consider using separate keys for development and production environments
  :::
